...
GITHUB_REPO = "/tmp/blacktech-archive"
...
def sync_to_github(entries):
    """Sync brain structure to GitHub repo."""
    import shutil
    
    # Ensure repo exists
    if not os.path.isdir(f"{GITHUB_REPO}/.git"):
        return ["🔴 GitHub repo not found at /tmp/blacktech-archive"]
    
    # Clean repo (keep .git, README.md, dewey_pipeline.py, dewey_sync.py)
    for item in os.listdir(GITHUB_REPO):
        if item == '.git':
            continue
        path = os.path.join(GITHUB_REPO, item)
        if os.path.isdir(path):
            shutil.rmtree(path)
        elif item not in ['README.md', 'dewey_pipeline.py', 'dewey_sync.py']:
            os.remove(path)
    ...
    # Copy catalog to root
    shutil.copy2(
        os.path.join(BRAIN, "000-General/dewey_catalog.json"),
        os.path.join(GITHUB_REPO, "dewey_catalog.json")
    )
    # Copy sync script
    shutil.copy2(
