...
def append_toll(task, cost):
    try:
        ...
        import toll_meter
        toll_meter.add_manual_entry(
            time.strftime("%Y-%m-%d"), task[:60], cost
        )
    except Exception as e:
        # Fallback — append to text file directly
        try:
...
- If you cannot complete it fully, say PARTIAL and summarize what is missing"""
    ...
    if gemini_ok and complexity == "low" and "PARTIAL" not in gemini_out and len(gemini_out) > MIN_OLLAMA_CHARS:
        ...
        # If Gemini failed, Ollama does the task from scratch instead of polishing garbage
        if not gemini_ok:
            print("  ↳ Gemini failed — Ollama handling task from scratch")
            ollama_prompt = f"""You are an expert electrical contractor and energy advisor in Chicago.

Complete this task professionally and concisely:

{user_task}

Rules:
- Provide a complete, usable answer
- No filler, no introductions
...
Produce the FINAL, professional, accurate response. This is the authoritative answer."""
    ...
    with open(out_file, "w") as f:
        ...
        f.write(f"GEMINI: {gemini_metrics['status'].upper()} -- ${gemini_metrics.get('cost',0):.6f}\n")
        f.write(f"OLLAMA: {ollama_metrics['status'].upper()} -- $0.00\n")
        f.write(f"CLAUDE: {claude_metrics['status'].upper()} -- ${claude_metrics.get('cost',0):.6f}\n")
